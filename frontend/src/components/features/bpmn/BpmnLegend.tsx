...
function LegendRow({
  ...
  return (
    <div className="flex gap-3 py-2.5 border-b border-border-app last:border-0">
      <div className="flex-shrink-0 w-[52px] h-11 flex items-center justify-center bg-surface-hover rounded-md border border-border-app">
        {icon}
      </div>
      <div className="min-w-0 flex-1 pt-0.5">
        <p className="text-[11px] font-semibold text-fg-primary leading-tight">{title}</p>
        <p className="text-[10px] text-fg-secondary leading-snug mt-1">{desc}</p>
      </div>
    </div>
  );
}
    ...
    <div ref={rootRef} className="relative z-[20]">
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-bpmn border transition-all shadow-sm ${
          open
            ? 'bg-surface-hover text-fg-primary border-border-app'
            : 'bg-surface-elevated text-fg-primary border-border-app hover:bg-surface-hover'
        }`}
      >
        <BookOpen size={14} className="text-fg-secondary shrink-0" aria-hidden />
        Legenda
      </button>
      ...
      {open && (
        <div
          ...
          className={`absolute top-full mt-2 w-[min(92vw,320px)] max-h-[min(72vh,520px)] overflow-y-auto rounded-bpmn border border-border-app bg-surface-elevated/98 backdrop-blur-sm shadow-md ${
            align === 'left' ? 'left-0' : 'right-0'
          }`}
        >
          <div className="sticky top-0 z-[1] flex items-start justify-between gap-2 px-3 py-2.5 border-b border-border-app bg-surface-elevated/95">
            <div>
              <p className="text-[11px] font-bold text-fg-primary tracking-tight">Legenda BPMN 2.0</p>
              <p className="text-[9px] text-fg-secondary mt-0.5 leading-snug">Símbolos usados neste fluxograma</p>
            </div>
            <button
              type="button"
              onClick={close}
              className="p-1 rounded-md text-fg-secondary hover:text-fg-primary hover:bg-surface-hover transition-colors"
          ...
          <div className="px-3 pb-3 pt-1">
            ...
            <p className="text-[9px] font-semibold uppercase tracking-[0.1em] text-fg-tertiary mt-3 mb-1">Tarefas</p>
            <LegendRow
              ...
              icon={
                <svg width="40" height="26" viewBox="0 0 48 28" className="text-fg-primary" aria-hidden>
                  <rect x="2" y="2" width="44" height="24" rx="6" fill="var(--surface-elevated)" stroke="currentColor" strokeWidth="1.2" />
            ...
            <p className="text-[9px] font-semibold uppercase tracking-[0.1em] text-fg-tertiary mt-3 mb-1">Gateways</p>
            <LegendRow
              title="Exclusive (XOR)"
              desc="Escolha de um único caminho (ex.: Sim ou Não)."
              icon={
                <svg width="36" height="36" viewBox="0 0 48 48" className="text-fg-primary" aria-hidden>
                  <polygon points="24,4 44,24 24,44 4,24" fill="var(--surface-elevated)" stroke="currentColor" strokeWidth="1.5" />
            ...
            <p className="text-[9px] font-semibold uppercase tracking-[0.1em] text-fg-tertiary mt-3 mb-1">Fluxos</p>
            <LegendRow
              title="Sequence Flow"
              desc="Ordem de execução entre elementos. Texto na linha indica condição (ex.: rótulo da decisão)."
              icon={
                <svg width="48" height="22" viewBox="0 0 56 24" className="text-fg-secondary" aria-hidden>
                  ...
                  <rect x="10" y="5" width="18" height="10" rx="2" fill="var(--surface-elevated)" stroke="currentColor" strokeWidth="0.8" />
            ...
            <LegendRow
              ...
              icon={
                <svg width="48" height="34" viewBox="0 0 56 36" className="text-fg-primary" aria-hidden>
                  ...
                  <line x1="12" y1="12" x2="55" y2="12" stroke="var(--fg-secondary)" strokeWidth="0.6" />
                  <line x1="12" y1="23" x2="55" y2="23" stroke="var(--fg-secondary)" strokeWidth="0.6" />
                </svg>
              }
            />
          </div>
